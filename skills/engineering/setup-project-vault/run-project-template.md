...
name: afk
description: >-
  Use this skill when the user invokes /afk or wants {{PROJECT_NAME}} driven
...
Drive {{PROJECT_NAME}} to completion in one session: repeatedly find every
...
4. **Dispatch the whole frontier in one message** — one subagent per ticket,
   ...
   2. Where `/slice` would stop for a human, close the loop instead: re-read
      ...
      and {{BUILD_CMD}}/{{LINT_CMD}}{{TEST_CLAUSE}} green is what stands in
...
5. **Integrate, one ticket at a time, in dispatch order** as each subagent
   ...
   `Done` move — then re-run {{BUILD_CMD}}/{{LINT_CMD}}{{TEST_CLAUSE}} on the
   integration branch to catch what only shows up combined. A conflict is
...
8. **Write the manual testing guide.** One file,
   `{{VAULT_DIR}}/Projects/{{PROJECT_SLUG}}/MANUAL-TESTING.md`: one section
